...
const files = [
  path.join(artifactsDir, "playwright-results.xml"),
  path.join(artifactsDir, "newman-results.xml"),
];
...
try {
  quarantine = JSON.parse(
    fs.readFileSync("quarantine/quarantine.json", "utf-8")
  );
} catch {}
for (const [k, v] of clusters.entries()) {
  if (quarantine.some((q) => v.name.includes(q))) {
    v.quarantined = true;
    clusters.set(k, v);
  }
}
...
fs.writeFileSync(
  path.join(artifactsDir, "fingerprints.json"),
